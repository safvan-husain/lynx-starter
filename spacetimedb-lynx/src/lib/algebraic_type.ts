...
export const ProductType = {
  makeSerializer(
    ...
    // Interpreter: iterate over elements and call each element's serializer.
    // Cache before recursing so recursive types have an exit condition.
    const elements = ty.elements;
    const serializersMap: Record<string, Serializer<any>> = {};
    serializer = (writer: BinaryWriter, value: any) => {
      for (const el of elements) {
        serializersMap[el.name!](writer, value[el.name!]);
      }
    };
    SERIALIZERS.set(ty, serializer);
    for (const el of elements) {
      serializersMap[el.name!] = AlgebraicType.makeSerializer(
        el.algebraicType,
        typespace
      );
    }
    Object.freeze(serializersMap);
    return serializer;
  },
  /** @deprecated Use `makeSerializer` instead. */
  serializeValue(
  ...
  makeDeserializer(
    ...
    // Interpreter: read tag and use lookup map to call variant deserializer.
    // Cache before recursing so recursive types have an exit condition.
    const elements = ty.elements;
    const deserializersMap: Record<string, Deserializer<any>> = {};
    deserializer = (reader: BinaryReader) => {
      const result: Record<string, any> = {};
      for (const el of elements) {
        result[el.name!] = deserializersMap[el.name!](reader);
      }
      return result;
    };
    DESERIALIZERS.set(ty, deserializer);
    for (const el of elements) {
      deserializersMap[el.name!] = AlgebraicType.makeDeserializer(
        el.algebraicType,
        typespace
      );
    }
    Object.freeze(deserializersMap);
    return deserializer;
  },
  /** @deprecated Use `makeDeserializer` instead. */
  deserializeValue(
...
export const SumType = {
  makeSerializer(ty: SumTypeType, typespace?: TypespaceType): Serializer<any> {
    if (
      ...
      // Interpreter: read tag and use lookup map to call variant serializer.
      const variants = ty.variants;
      const serializersMap: Record<string, Serializer<any>> = {};
      serializer = (writer: BinaryWriter, value: any) => {
        const i = variants.findIndex(v => v.name === value.tag);
        if (i < 0) {
          throw new TypeError(
        ...
        writer.writeByte(i);
        serializersMap[variants[i].name!](writer, value.value);
      };
      SERIALIZERS.set(ty, serializer);
      for (const v of variants) {
        serializersMap[v.name!] = AlgebraicType.makeSerializer(
          v.algebraicType,
          typespace
        );
      }
  ...
  makeDeserializer(
    ...
    if (
      ...
      deserializer = (reader: BinaryReader) => {
        ...
        if (variant == null) {
          throw new TypeError(
            `Could not deserialize sum type; unknown tag ${tag}`
          );
        }
        return {
          tag: variant.name!,
          value: deserializersMap[variant.name!](reader),
        };
      };
      DESERIALIZERS.set(ty, deserializer);
      for (const v of variants) {
        deserializersMap[v.name!] = AlgebraicType.makeDeserializer(
          v.algebraicType,
          typespace
        );
      }
